...
prompt_pure_preprompt_render() {
	...
	# Set color for git branch/status change color if dirty checking has been delayed.
	local git_color=242
...
prompt_pure_async_refresh() {
	...
	if (( time_since_last_status_check > ${PURE_GIT_DELAY_STATUS_CHECK:-1800} )); then
		unset prompt_pure_git_last_status_check_timestamp
		# check check if there is anything to pull
		async_job "prompt_pure" prompt_pure_async_git_status ${PURE_GIT_UNTRACKED_STATUS:-1} $PWD
	fi
}
...
prompt_pure_async_callback() {
	...
	case $job in
		...
		prompt_pure_async_git_status)
			...
			if (( code == 0 )); then
				typeset -g prompt_pure_git_status="%F{green}✓"
			else
				typeset -g prompt_pure_git_status="%F{red}✗"
			fi
			...
			(( $exec_time > 5 )) && prompt_pure_git_last_status_check_timestamp=$EPOCHSECONDS
			;;
	esac
